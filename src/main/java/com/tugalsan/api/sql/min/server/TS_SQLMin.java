...
public class TS_SQLMin {
    ...
    final private TS_SQLMinExecutor executor;
     ...
     public TS_SQLMinValue whereGroupAnd(TGS_Func_In1<TS_SQLWhereGroups> groups) {
        executor.where = TS_SQLWhereUtils.where();
        executor.where.groupsAnd(groups);
        return new TS_SQLMinValue(executor);
    }

    public TS_SQLMinValue whereGroupOr(TGS_Func_In1<TS_SQLWhereGroups> groups) {
        executor.where = TS_SQLWhereUtils.where();
        executor.where.groupsOr(groups);
        return new TS_SQLMinValue(executor);
    }

    public TS_SQLMinValue whereConditionAnd(TGS_Func_In1<TS_SQLWhereConditions> conditions) {
        return whereGroupAnd(where -> where.conditionsAnd(conditions));
    }

    public TS_SQLMinValue whereConditionOr(TGS_Func_In1<TS_SQLWhereConditions> conditions) {
